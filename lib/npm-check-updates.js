...
function printTable(options, message, loglevel, method = 'log') {
    // not in json mode
    // not silent
 ...
 */
function printUpgrades(options, args, show = true) {
    ...
    // ToDo this code seems wrong, there is parameter mismatch
        ...
        if (show) {
            printTable(options, table.toString());
        }
    }
}
...
/** Finds the package file and data.
    @return Promise [pkgFile, pkgData]
